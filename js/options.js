...
function tabClick(id) {
    if(currentTab != id) {
        ...
        switch(id) {
            ...
            case 6:
                // Load Feedback Form
                document.getElementById("feedback-form").src = "https://docs.google.com/forms/d/e/1FAIpQLSfVzqj4RN8Jpa_xlWPrZxNiyUMX7KrnkLlLYF4ulzt13kEX7Q/viewform?embedded=true";
                break;
        }
    }
    scroll(0,0);
...
function Test() {
    storage.loadJSON("/js/test.json").then((value) => {
        ...
        for(let i = 0; i < value.unapproved.length; i++) {
            ...
            acu.exceptionsInOffice += ( +value.exceptionsInOffice[i].split(":")[1] + (+value.exceptionsInOffice[i].split(":")[0] * 60) );
        }
