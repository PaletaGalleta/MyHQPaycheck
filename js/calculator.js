...
export function init() {
    ...
    // Click listeners
    containerCalc.querySelector("#period-first").addEventListener("click", () => {
        changePeriod(0);
    });
    containerCalc.querySelector("#period-prev").addEventListener("click", () => {
        changePeriod(selectedPeriod - 1);
    });
    containerCalc.querySelector("#period-next").addEventListener("click", () => {
        changePeriod(selectedPeriod + 1);
    });
    containerCalc.querySelector("#period-last").addEventListener("click", () => {
...
export async function changePeriod(chg) {
        ...
        datasets: [
            {
                label: "Minutes",
                data: [0, 0, 0, 0],
                backgroundColor: ["rgb(54, 162, 235)", "rgb(255, 99, 132)", "rgb(105, 205, 86)", "#64ffb1"],
                borderWidth: 0,
                hoverOffset: 10,
    ...
    document.getElementById("period-lapse").innerHTML =
        moment(periodDate.startDate, "DD-MM-YYYY").format("MMMM DD") +
        " - " +
        moment(periodDate.endDate, "DD-MM-YYYY").format("DD");
    ...
    storage.loadJSON("/js/hr.json").then(hr => {
        storage.getDataFromLocalStorage("settings").then(setting => {
            ...
            // Display Rates
            containerCalc.querySelector("#rate-scheduled").innerHTML =
                "$" + (Math.round(breakdown.rate.schedule * 100) / 100).toFixed(2);
            ...
            containerCalc.querySelector("#rate-overtime").innerHTML =
                "$" + (Math.round(breakdown.rate.overtime * 100) / 100).toFixed(2);
            containerCalc.querySelector("#rate-discounts").innerHTML =
                "-$" + (Math.round(breakdown.rate.ap * 100) / 100).toFixed(2);
            ...
            containerCalc.querySelector("#hrs-overtime").innerHTML = (Math.round(breakdown.counter.overtime * 100) / 100).toFixed(
                2
            );
            containerCalc.querySelector("#hrs-discounts").innerHTML = "-" + breakdown.counter.ap;

            // Display Totals
            containerCalc.querySelector("#total-scheduled").innerHTML =
                "$" + (Math.round(breakdown.total.schedule * 100) / 100).toFixed(2);
            ...
            containerCalc.querySelector("#total-overtime").innerHTML =
                "$" + (Math.round(breakdown.total.overtime * 100) / 100).toFixed(2);
            containerCalc.querySelector("#total-discounts").innerHTML =
                "$" + (Math.round(breakdown.total.ap * 100) / 100).toFixed(2);

            containerCalc.querySelector("#total-hours").innerHTML = (Math.round(breakdown.totalHours * 100) / 100).toFixed(2);
            containerCalc.querySelector("#total-total").innerHTML =
                "$" + (Math.round(breakdown.grandTotal * 100) / 100).toFixed(2);
            containerCalc.querySelector("#total-paycheck").innerHTML =
                "$" + (Math.round(breakdown.grandTotal * 100) / 100).toFixed(2);
...
async function countMinutes(periodDate) {
    ...
    for (let i = 0; i < dif + 1; i++) {
        ...
        const promise = new Promise(resolve => {
            ...
            settings.getShift(moment(tmpDate).add(i, "days").format("DD-MM-YYYY")).then(shift => {
