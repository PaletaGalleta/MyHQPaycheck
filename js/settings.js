...
export function load() {
  ...
  storage.getDataFromLocalStorage("settings").then( (value) => {
    ...
    if(!value.empty) {
      ...
      document.getElementById("startwork").value = (savedSettings.jobStart) ? moment(savedSettings.jobStart, "DD-MM-YYYY").format("YYYY-MM-DD") : "";
      document.getElementById("checkdev").checked = (savedSettings.devmode) ? true : false;
      document.getElementById("welcome").innerHTML = (savedSettings.nickname) ? "Welcome, " + savedSettings.nickname : "Welcome, User";
    }
    else {
      savedSettings = {};
      document.getElementById("missinginfo").style.display = "flex";
  ...
  document.getElementById("settingsform").addEventListener("submit", function(evt) {
    evt.preventDefault();
    saveInfo()
  }, true);
}
    ...
    storage.loadJSON("/js/hr.json").then((val) => {
      ...
      for(let i = 0; i < val.validEmails.length; i++) {
        ...
        if(emailenc == val.validEmails[i]) {
          ...
          // Welcome the user
          notifications.showToast("Welcome, " + email + "! You have unlimited access to the Beta Version, enjoy!", "success")
          // Hide Modal
          showBetaModal(false);
          // Exit
          return;
      ...
      notifications.showToast("Sorry, the email didn't get any coincidences. Be sure to type the email you registered for the Beta, and remember to type it in lowercase", "danger");
    });
  });
...
function saveInfo() {
  ...
  if(strtJb != "") {
    ...
    if(moment().diff(moment(savedSettings.jobStart, "DD/MM/YYYY"), "days") >= 365) achievements.setProgress("oneYear", 365, true);
  }
